# %% [markdown]
# # Sentiment Analysis: LoRA Llama 3.2 1B
# ## Fine-tuning for Classification with PEFT
...
class Trainer:
    def __init__(self, model: nn.Module, config: Config, total_steps: int):
        ...
        # Only optimize trainable parameters (LoRA params + classifier)
    ...
    def _apply_lora(self, model: nn.Module) -> nn.Module:
        # Apply LoRA to the Llama model (not the classifier head)
        ...
        # Apply LoRA only to the llama part, not the classifier
...
if __name__ == "__main__":
    ...
    print("Llama 3.2 1B Sentiment Analysis - LoRA")
    print("=" * 60)
    ...
    # Save results
    results_file = f"results_lora_{int(time.time())}.json"
