...
class ModelPrinter:
    ...
    def print_model_architecture(model: nn.Module, model_name: str = "Model"):
        ...
        print("\n" + "-" * 70)
        ModelPrinter.print_parameter_summary(model)
        print("=" * 70)
...
class AutoTrainer:
    ...
    def save_checkpoint(self, epoch: int, val_loss: float, val_acc: float):
        ...
        filename = f"baseline_epoch{epoch}_loss{val_loss:.4f}_acc{val_acc:.4f}.pt"
...
def plot_training_curves_seaborn(trainer: AutoTrainer):
    """Plot training curves using Seaborn"""
    # Prepare data for seaborn
    epochs = list(range(len(trainer.metrics["train_loss"])))
    ...
    for epoch in epochs:
        data_list.append(
            {
                "Epoch": epoch,
                "Loss": trainer.metrics["train_loss"][epoch],
                "Accuracy": trainer.metrics["train_acc"][epoch],
                "Type": "Train",
            }
        )
        data_list.append(
            {
                "Epoch": epoch,
                "Loss": trainer.metrics["val_loss"][epoch],
                "Accuracy": trainer.metrics["val_acc"][epoch],
                "Type": "Validation",
            }
        )
    ...
    # Loss convergence analysis (log scale)
    axes[1, 0].semilogy(
        epochs,
    ...
    axes[1, 0].semilogy(
        epochs, trainer.metrics["val_loss"], label="Validation", marker="s", alpha=0.7
    )
    axes[1, 0].set_xlabel("Epoch")
    axes[1, 0].set_ylabel("Log Loss")
    axes[1, 0].set_title("Loss Convergence (Log Scale)", fontweight="bold")
    axes[1, 0].legend()
    ...
    performance_data = pd.DataFrame(
        {
            "Train": trainer.metrics["train_acc"],
            "Validation": trainer.metrics["val_acc"],
        }
    )

    # Melt for seaborn
    melted = performance_data.melt(var_name="Dataset", value_name="Accuracy")
    ...
    plt.suptitle("Training Analysis: Baseline MLP", fontsize=16, fontweight="bold", y=1.02)
    plt.tight_layout()
    plt.show()
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("MNIST TRAINING: BASELINE MLP")
    print("=" * 60)
    ...
    baseline_trainer = AutoTrainer(
        baseline_model, config, experiment_name="baseline_mlp"
    )
    ...
    print(f"\nBaseline Test Loss: {baseline_test_metrics['loss']:.4f}")
    print(f"Baseline Test Accuracy: {baseline_test_metrics['acc']:.4f}")

    # %%
    # Visualize results
    plot_training_curves_seaborn(baseline_trainer)
    ...
    print("\nFinal Results:")
    print(f"  - Test Loss: {baseline_test_metrics['loss']:.4f}")
    print(f"  - Test Accuracy: {baseline_test_metrics['acc']:.4f}")
    print(f"  - Best Validation Loss: {baseline_trainer.best_val_loss:.4f}")
    print(f"  - Best Validation Accuracy: {baseline_trainer.best_val_acc:.4f}")
